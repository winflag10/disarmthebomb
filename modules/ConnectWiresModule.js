class ConnectWiresModule{
	constructor(uuid, lifeModule){
		...
		var ConnectWiresModuleHTML = `
		<div class="connect-wires-module module" id="module-${this.uuid}">
			<div class="win-cover" id="win-cover-${this.uuid}"></div>
			<div class="connect-wires-container">
				<div class="connect-wires-slot-section connect-wires-section-left" id="connect-wires-section-0-${this.uuid}"></div>
				<div class="connect-wires-slot-section connect-wires-section-right" id="connect-wires-section-1-${this.uuid}"></div>
				<div class="connect-wires-back cursor" onclick="activeModules[${this.uuid}].resetLines()"><i class="fas fa-undo fa-2x"></i></div>
				<div class="connect-wires-check cursor" onclick="activeModules[${this.uuid}].check()"><i class="fas fa-check fa-2x"></i></div>
				<canvas class="connect-wires-canvas" id="connect-wires-canvas-${this.uuid}"></canvas>
			</div>
		</div>`

		this.defused = 0
		document.getElementById("game").innerHTML += ConnectWiresModuleHTML
		...
		for(let i=1;i<5;i++){
			document.getElementById(`connect-wires-section-0-${this.uuid}`).innerHTML += `<div class="connect-wires-wire-slot cursor" id="connect-wires-slot-${this.uuid}-${i}" style="background:${this.colours[i-1]};" onclick="activeModules[${this.uuid}].slotClick(${i})"></div>`
		}
		for(let i=5;i<9;i++){
			document.getElementById(`connect-wires-section-1-${this.uuid}`).innerHTML += `<div class="connect-wires-wire-slot cursor" id="connect-wires-slot-${this.uuid}-${i}" style="background:${this.colours[i-1]};" onclick="activeModules[${this.uuid}].slotClick(${i})"></div>`
		}
	...
	defuse(){
		...
		//this.drawLines()
		for(let i=1;i<9;i++){
			document.getElementById(`connect-wires-slot-${this.uuid}-${i}`).style.backgroundColor = "#09ec09"
		}
		...
		if([0,1,2,3,4].includes(i)){
			if(this.selected == i){
				...
				for(let x=0;x<4;x++){
					let y = x+1
					if(y == this.selected){
						document.getElementById(`connect-wires-slot-${this.uuid}-${y}`).style.opacity = 0.8
					}else{
						document.getElementById(`connect-wires-slot-${this.uuid}-${y}`).style.opacity = 1
					}
				}
			}
		}else{
	...
	resetSelected(){
		for(let x=1;x<5;x++){
			document.getElementById(`connect-wires-slot-${this.uuid}-${x}`).style.opacity = 1
		}
	}

	drawLines(){
		var c = document.getElementById(`connect-wires-canvas-${this.uuid}`);
